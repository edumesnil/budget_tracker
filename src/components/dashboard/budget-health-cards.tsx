...
function CategoryRow({ entry, spent }: { entry: MergedBudget; spent: number }) {
...
function GroupCard({ group, spendMap }: { group: BudgetGroup; spendMap: Map<string, number> }) {
      ...
      >
        ...
        >
          ...
          <div className={css({ display: "flex", alignItems: "center", gap: "3" })}>
            <div className={css({ textAlign: "right" })}>
              <span className={css({ fontSize: "sm", fontWeight: "600" })} style={{ color }}>
        ...
        {status !== "ok" && (
          <div className={css({ display: "flex", justifyContent: "flex-end" })}>
            <span className={css({ fontSize: "xs", fontWeight: "500" })} style={{ color }}>
              {status === "over"
