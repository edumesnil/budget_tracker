...
function getPrior6Months(month: number, year: number): Array<{ month: number; year: number; label: string }> {
  const result = []
  for (let i = 5; i >= 0; i--) {
    let m = month - i
    let y = year
...
export function SpendingTrend({ transactions, currentMonth, currentYear }: Props) {
  const periods = useMemo(() => getPrior6Months(currentMonth, currentYear), [currentMonth, currentYear])
    ...
    return (
      ...
      >
        ...
        <p className={css({ fontSize: 'xs' })}>
          Trend shows after 2+ months of transactions.
        </p>
      </div>
    )
  }
