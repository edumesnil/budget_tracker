...
export default function CategoriesPage() {
  ...
  const handleGroupSubmit = async (data: { name: string; icon?: string; color?: string }) => {
    try {
      if (editingGroup) {
        ...
        toaster.success({ title: 'Group updated', description: `"${data.name}" has been updated.` })
      } else {
      ...
      toaster.error({ title: 'Error', description: `Failed to ${editingGroup ? 'update' : 'create'} group.` })
    }
  }
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    try {
      ...
      toaster.error({ title: 'Error', description: 'Failed to delete category. Please try again.' })
    }
  }
  ...
  const handleCategorySubmit = async (data: {
    ...
    try {
      if (editingCategory) {
        ...
        toaster.success({ title: 'Category updated', description: `"${data.name}" has been updated.` })
      } else {
        await createCategory.mutateAsync(data)
        toaster.success({ title: 'Category created', description: `"${data.name}" has been added.` })
      }
      setCategoryDialogOpen(false)
    } catch {
      toaster.error({ title: 'Error', description: `Failed to ${editingCategory ? 'update' : 'create'} category.` })
    }
  }
